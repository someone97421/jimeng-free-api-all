...
- **控制台改为本地免登录访问**：移除登录页、初始化账号、Session 校验和密码管理，打开控制台地址即可直接使用统计、日志和媒体管理功能。
- **新增本地媒体落盘**：图片和视频生成结果自动保存到 `data/media`，控制台优先读取本地文件，并自动保留最近 50 条产物、清理更早文件；原始远程 URL 继续保留用于回退。
...
- **可视化管理**：内置无需登录的 Web 管理控制台，支持实时监控、媒体管理和本地落盘预览。
...
| **🖼️ 媒体回溯** | 所有生成的图片和视频都会记录在媒体库并自动落盘，支持按类型筛选，点击即可预览高清原图或播放视频。 |
| **💾 本地存储** | 产物保存于 `data/media`，全局保留最近 50 条图片/视频文件；超出数量后自动删除旧文件，但保留历史记录和远程 URL。 |
| **💳 积分追踪** | **[NEW]** 每次请求自动计算并记录消耗积分，实时更新 Token 的剩余积分，精准掌握额度使用情况。               |

### 🚀 访问地址

部署成功后，访问：`http://localhost:8001`（默认端口）即可直接进入管理控制台，无需登录。

> ⚠️ 控制台按本地服务设计，统计、日志和媒体记录接口不再要求登录。请勿将管理端口直接暴露到不可信网络。
...
- **混合生成**：支持 1-10 张参考图生成（Image-to-Image），可通过兼容参数 `filePath` 传单张，也可通过 `filePaths` / `file_paths` / `images` 数组或 multipart 文件上传传多张。
...
- `-v jimeng-data:/app/data`：**强烈建议挂载**，用于持久化保存统计数据、媒体记录、日志和本地产物文件。
- `-e TZ=Asia/Shanghai`：设置时区，确保日志和统计时间准确。
- `MEDIA_DIR`：本地媒体目录，默认 Docker 路径为 `/app/data/media`。
- `MEDIA_RETENTION_COUNT`：保留的最近产物数量，默认 `50`。
- `MEDIA_MAX_FILE_SIZE`：单个产物大小上限，默认 `536870912` 字节（512 MiB）。
...
| 模型名称               | 说明                      | 分辨率    |
| :--------------------- | :------------------------ | :-------- |
| `jimeng-image-5.0-pro`  | 即梦 5.0 Pro 版本（动态同步） | 1K / 2K (默认) / 4K |
